...
public class MuFormularioBean implements Serializable,IControlPrivilegios {
	...
	public void editFormulario() {
		if (formulario != null) {
			...
			idFormMod = formulario.getFormularioId();
			tipoVista = (formulario.getFormularioId() == null);
			edit = true;
	...
	private void saveModulo() throws Exception{
		this.formulario.setFormularioId(null);
	...
	public void deleteInterfaz() {
		if(formulario != null){
			try {
				...
				log.error("[deleteInterfaz]  error al eliminar el Formulario[" + formulario.getId()+"],"+ e.getMessage(),e);
