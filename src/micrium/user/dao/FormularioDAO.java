...
public class FormularioDAO implements Serializable {
	...
	public List<Object[]> findPadresPermisos(long rolId) {
		String sql = "SELECT F.id, F.nombre, CASE WHEN (SELECT FF.id FROM MuFormulario FF, MuRolFormulario RR where FF.id = RR.id.formularioId "
				+ "and RR.id.rolId = :rolId AND FF.estado = true AND RR.estado = true AND F.id = FF.id) IS NULL THEN 0 ELSE 1 END  as permiso "
				+ "FROM MuFormulario F, MuRolFormulario R where F.id = R.id.formularioId "
				+ "and F.formularioId is null and R.id.rolId = 1 AND F.estado = true AND R.estado = true  order by F.orden";
	...
	public List<Object[]> findHijosPermisos(long raizId, long rolId) {
		String sql = "SELECT F.id, F.nombre, CASE WHEN (SELECT FF.id FROM MuFormulario FF, MuRolFormulario RR where FF.id = RR.id.formularioId "
				+ "and RR.id.rolId = :rolId AND FF.estado = true AND RR.estado = true AND F.id = FF.id) IS NULL THEN 0 ELSE 1 END  as permiso "
				+ "FROM MuFormulario F, MuRolFormulario R where F.id = R.id.formularioId "
				+ "and F.formularioId = :raizId and R.id.rolId = 1 AND F.estado = true AND R.estado = true  order by F.orden";
		Query query = entityManager.createQuery(sql);
		query.setParameter("raizId", raizId);
		query.setParameter("rolId", rolId);
		return query.getResultList();
